...
void input(pt &ref, std::vector<orf> &orfs, std::vector<pt> &dat) {
	std::string buf; // buffer
	// read reference genomic data
	std::fstream ref_file; // reference (NC_045512.2)
	ref_file.open("ref (NC_045512.2).fasta", std::fstream::in);
	if(!ref_file.is_open())
	...
	orf_file.open("ORF_file.txt", std::fstream::in);
	if(!orf_file.is_open())
		std::cout << "reference genome data reading error" << std::endl;
	while(!orf_file.eof()) {
		orf orf_tmp;
...
bool find_mutation(pt &ref, std::vector<orf> &orfs, pt &cmp, std::vector<int> &loc, std::vector<std::string> &pri, std::vector<std::vector<int>> &kmp_tab, std::vector<int> &pms) {
	...
	for(int i = 0; i < orfs.size(); ++i) {
		...
		for(int j = orfs[i].bg - 1; j < orfs[i].ed && j < ref.seq.length() && j + sft[i] < cmp.seq.length(); ++j) {
			if(cmp.seq[j + sft[i]] != 'A' && cmp.seq[j + sft[i]] != 'C' && cmp.seq[j + sft[i]] != 'G' && cmp.seq[j + sft[i]] != 'T')
				continue;
			if(ref.seq[j] != cmp.seq[j + sft[i]] && j + 1 >= 289) { // no mutations important before 289
...
int main()
{
	...
	out_file.open("input_file.txt", std::fstream::out); // input file for Project 2
	std::vector<int> pms;
